...
import MinimalMode from "../components/ui/MinimalMode"
...
const Queue: React.FC<QueueProps> = ({ setView }) => {
  ...
  const handleChatSend = async () => {
    ...
    // Add chat context
    await window.electronAPI.addChatContext(chatInput)
    
    try {
      const audioContext = await window.electronAPI.getAudioContextFromMonitor()
      const response = await window.electronAPI.invoke("gemini-chat", chatInput, audioContext)
  ...
  const toggleAudioMonitoring = async () => {
    try {
      if (audioMonitoring) {
        ...
        // Start browser-side audio capture
        if (!audioCaptureRef.current) {
          audioCaptureRef.current = new AudioCapture()
          await audioCaptureRef.current.startCapture(async (chunk) => {
            await window.electronAPI.addAudioChunk(chunk)
            // Get transcription
            const result = await window.electronAPI.analyzeAudioFromBase64(chunk.data, chunk.mimeType)
            await window.electronAPI.addTranscription(result)
          })
        }
      }
      setAudioMonitoring(!audioMonitoring)
    } catch (error) {
      console.error('Error toggling audio monitoring:', error)
      showToast('Error', 'Failed to toggle audio monitoring', 'error')
    }
  }
  ...
  const toggleScreenMonitoring = async () => {
    try {
      ...
      setScreenMonitoring(!screenMonitoring)
    } catch (error) {
      console.error('Error toggling screen monitoring:', error)
      showToast('Error', 'Failed to toggle screen monitoring', 'error')
    }
  }

  const handleQuickAction = async (action: 'screenshot' | 'audio' | 'chat') => {
    switch (action) {
      case 'screenshot':
        await window.electronAPI.takeScreenshot()
        break
      case 'audio':
        await toggleAudioMonitoring()
        break
      case 'chat':
        setIsChatOpen(true)
        break
          ...
          {isSettingsOpen && (
            <div className="mt-4 w-full mx-auto">
              ...
              <div className="mt-4 space-y-3">
                <div className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg">
                  ...
                  <button
                    ...
                    className={`w-12 h-6 rounded-full transition-colors ${
                      audioMonitoring ? 'bg-green-500' : 'bg-gray-600'
                    }`}
                  >
                    <div className={`w-5 h-5 bg-white rounded-full transition-transform ${
                      audioMonitoring ? 'translate-x-6' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>
                
                <div className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg">
                  <span className="text-sm text-gray-300">Screen Monitoring</span>
                  <button
                    onClick={toggleScreenMonitoring}
                    className={`w-12 h-6 rounded-full transition-colors ${
                      screenMonitoring ? 'bg-blue-500' : 'bg-gray-600'
                    }`}
                  >
                    <div className={`w-5 h-5 bg-white rounded-full transition-transform ${
                      screenMonitoring ? 'translate-x-6' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>
                
                <div className="flex items-center justify-between px-3 py-2 bg-white/5 rounded-lg">
                  <span className="text-sm text-gray-300">Minimal Mode</span>
                  <button
                    onClick={() => setMinimalMode(!minimalMode)}
                    className={`w-12 h-6 rounded-full transition-colors ${
                      minimalMode ? 'bg-purple-500' : 'bg-gray-600'
                    }`}
                  >
                    <div className={`w-5 h-5 bg-white rounded-full transition-transform ${
                      minimalMode ? 'translate-x-6' : 'translate-x-0.5'
                    }`} />
                  </button>
                </div>
                ...
                <AdvancedSettings 
                  trigger={
                    <button className="w-full px-4 py-2 bg-blue-500 hover:bg-blue-600 rounded text-sm transition-colors">
                      Advanced Settings
                    </button>
                  }
                />
